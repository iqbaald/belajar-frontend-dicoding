...
//pengisian value ke web storage
window.addEventListener("load", function () {
  if (typeof Storage !== "undefined") {
    ...
    alert("Sayangnya browser yang anda gunakan tidak mendukung web storage");
  }
...
function updateScore() {
  ...
  if (sessionAttempsValue > localAttempsValue) {
    localStorage.setItem(localMaximumAttemptsKey, sessionAttempsValue);
    localMaximumAttemptField.innerText = sessionAttempsValue;
  }
...
window.addEventListener("beforeunload", function () {
  ...
  sessionStorage.setItem(sessionUserAttemptsKey, 0);
  sessionUserAnswerField.innerText = sessionUserAttemptsKey;
});
